...
export default function ProfileCard() {
    const onGotoClick = () => window.open('https://www.linkedin.com/in/kevin-yang-aba2a830/', '_blank')
    ...
    return (
        <div className={`card filter backdrop-blur-xl m-auto w-[350px] shadow-2xl bg-slate-900 p-6 rounded-xl`}>
          ...
          <button 
            className="h-10 px-6 mt-5 rounded-md border text-sm text-slate-800 bg-white font-bold hover:bg-transparent hover:text-white" 
            type="button"
            onClick={onGotoClick}
          >
            Contact Me
